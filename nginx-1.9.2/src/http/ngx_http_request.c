...
void //ngx_http_run_posted_requests函数是处理子请求的。 //ngx_http_post_request将该子请求挂载在主请求的posted_requests链表队尾，在ngx_http_run_posted_requests中执行
...
{ //subrequest注意ngx_http_run_posted_requests与ngx_http_postpone_filter ngx_http_finalize_request配合阅读
    ...
    //如果优先级低的子请求的数据线到达，则先通过ngx_http_postpone_filter->ngx_http_postpone_filter_add缓存到r->postpone，
    ...
    for ( ;; ) {
        ...
        /*
        根据ngx_http_request_t结构体中的main成员找到原始请求，这个原始请求的posted_requests成员指向待处理的post请求组成的单链表，
        如果posted_requests指向NULL空指针，则结束ngx_http_run_posted_requests方法，否则取出链表中首个指向post请求的指针
          */
        if (pr == NULL) { //为NULL说明没有子请求
            return;
        }
