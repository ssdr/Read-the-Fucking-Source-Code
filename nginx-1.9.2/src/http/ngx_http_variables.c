...
3. 将命名子模式加入到cmcf->variables_keys和cmcf->variables中。以备后续通过名字查找变量值。*/
ngx_http_regex_t *
...
//用已经编译的regex跟e->line,也就是s去匹配，看看是否匹配成功。如果匹配成功，整理存放在r->variables里面
ngx_int_t
        ...
        for (n = 0; n < cmcf->variables_keys->keys.nelts; n++) {
            ...
            if (v[i].name.len == key[n].key.len
                && ngx_strncmp(v[i].name.data, key[n].key.data, v[i].name.len)
                   == 0)
            { //对于合法的使用变量会将其对应的三个主要字段设置好，即get_handler回调、data数据、flags旗标
