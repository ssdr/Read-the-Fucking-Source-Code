...
/*可以调用ngx_http_set_content_type(r)方法帮助我们设置Content-Type头部，这个方法会根据URI中的文件扩展名并对应着mime.type来设置Content-Type值,取值如:image/jpeg*/
    ...
    off_t                             content_length_n; //这个标示应答体的长度
    time_t                            date_time;
...
typedef struct {//在ngx_http_read_client_request_body中分配存储空间
    ...
    //存放HTTP包体的临时文件  ngx_http_write_request_body分配空间  //如果配置"client_body_in_file_only" on | clean 表示包体存储在磁盘文件中
    ...
    //在ngx_http_read_client_request_body中赋值，执行在ngx_http_do_read_client_request_body
...
//通过ngx_http_upstream_cache_get获取
    ...
    */
     ...
     */
    ngx_pool_t                       *pool;
    //其中，header_in指向Nginx收到的未经解析的HTTP头部，这里暂不关注它（在第11章中可以看到，header_in就是接收HTTP头部的缓冲区）。 header_in存放请求行，Headers_in存放头部行
    //请求行和请求头部内容都在该buffer中
    ngx_buf_t                        *header_in;//用于接收HTTP请求内容的缓冲区，主要用于接收HTTP头部，该指针指向ngx_connection_t->buffer
    //类型的headers_in则存储已经解析过的HTTP头部。下面介绍ngx_http_headers_in_t结构体中的成员。
/*
...
//内部重定向是从NGX_HTTP_SERVER_REWRITE_PHASE处继续执行(ngx_http_internal_redirect)，而重新rewrite是从NGX_HTTP_FIND_CONFIG_PHASE处执行(ngx_http_core_post_rewrite_phase)
    ...
    unsigned                          allow_ranges:1;  //支持断点续传
...
//request_start与method_end的用法也很简单，其中request_start指向用户请求的首地址，同时也是方法名的地址，method_end指向方法名
//的最后一个字符（注意，这点与其他xxx_end指针不同）。获取方法名时可以从request_start开始向后遍历，直到地址与method_end相同为止，这段内存存储着方法名。
